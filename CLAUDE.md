...
- `dotnet test src/RadioCmCutter.Core.Tests/...` → 30件全て成功（FFT・繰り返し検出・区間補集合計算・カット位置候補検出・境界補正・区間ごとのCM判定・複数ファイル一括処理でのファイル跨ぎ検出の単体テスト）。テストは実音声を使わず、合成した`FrameFeatures`・`DecodedAudio`を手組みする方式（`RepeatSegmentDetectorTests`の慣習に倣う）
